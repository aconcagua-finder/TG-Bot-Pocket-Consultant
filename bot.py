...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ask_question":
        ...
        await query.edit_message_text(
            ...
            f"Осталось обработок документов сегодня: <b>{DAILY_DOCUMENT_LIMIT - user_limits_obj.documents_count}</b>\n\n"
            "📝 <b>Шаг 1:</b> Сначала напишите, что именно нужно изменить в документе.\n\n"
            "<b>Примеры:</b>\n"
            "• \"Изменить 2024 год на 2025\"\n"
            "• \"Улучшить юридическую формулировку\"\n"
            "• \"Исправить грамматические ошибки\"\n"
            "• \"Сделать текст более официальным\"\n\n"
            "💡 <b>Чем конкретнее, тем лучше результат!</b>\n\n"
            "⚠️ <b>Поддерживаемые форматы:</b>\n"
            "• Текстовые файлы (.txt) ✅\n"
            "• Документы Word (.docx) ✅\n"
            "• PDF файлы (.pdf) ❌ (только анализ)\n\n"
            "Напишите ваши требования к редактированию:",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="back_to_main")]])
        )
        context.user_data['waiting_for'] = 'edit_instructions'
        ...
        context.user_data.pop('edit_instructions', None)
        await query.edit_message_text(
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'edit_instructions':
        # Сохраняем инструкции пользователя для редактирования
        context.user_data['edit_instructions'] = message_text
        log_user_action(user.id, user.username or "Unknown", "edit_instructions", message_text)
        
        await update.message.reply_text(
            "✅ <b>Инструкции сохранены!</b>\n\n"
            f"📝 <b>Что нужно сделать:</b> {message_text}\n\n"
            "📎 <b>Шаг 2:</b> Теперь отправьте документ для редактирования.\n\n"
            "⚠️ <b>Ограничения:</b>\n"
            "• Максимальный размер: 20 МБ\n"
            "• Форматы: .txt, .docx\n\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if waiting_for == 'analyze_document':
            ...
            user_instructions = context.user_data.get('edit_instructions', 'улучши документ')
            
            prompt = f"""Ты юрист-редактор. Твоя задача - отредактировать этот юридический документ согласно требованиям пользователя.

ТРЕБОВАНИЯ ПОЛЬЗОВАТЕЛЯ: {user_instructions}
...
4. ОБЯЗАТЕЛЬНО выполни требования пользователя
5. Если пользователь просит изменить даты, цифры, названия - сделай это
6. Сохрани структуру документа, но внеси нужные изменения
7. Улучши юридическую точность и ясность

Просто пришли отредактированную версию документа согласно требованиям пользователя:"""
        ...
        if waiting_for == 'edit_document':
            user_instructions = context.user_data.get('edit_instructions', 'улучши документ')
            logger.info(f"User edit instructions: {user_instructions}")
