            ...
            if response:
                ...
                for paragraph_text in paragraphs:
                    if paragraph_text.strip():
                        # Если это заголовок (все заглавными или содержит слова типа "ДОГОВОР", "ЗАЯВЛЕНИЕ")
                        if (paragraph_text.isupper() and len(paragraph_text) < 100) or \
                           any(word in paragraph_text.upper() for word in ['ДОГОВОР', 'СОГЛАШЕНИЕ', 'ЗАЯВЛЕНИЕ', 'ПРЕТЕНЗИЯ', 'ДОВЕРЕННОСТЬ', 'УВЕДОМЛЕНИЕ']):
